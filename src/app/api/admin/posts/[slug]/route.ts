...
import { deleteStoredPost, readStoredPosts, upsertStoredPost } from "@/lib/blog-store";
...
function parsePostBody(body: unknown, currentSlug: string): BlogPostInput | { error: string } {
  ...
  if (slug !== currentSlug) {
    return { error: "Slug cannot be changed after publishing." };
  }
...
export async function GET(_request: Request, context: RouteContext) {
  ...
  const post = (await readStoredPosts()).find((entry) => entry.slug === slug);
  if (!post) {
    return NextResponse.json({ error: "Post not found in admin store." }, { status: 404 });
  }
...
export async function PUT(request: Request, context: RouteContext) {
  ...
  const { slug } = await context.params;
  const existing = (await readStoredPosts()).find((entry) => entry.slug === slug);
  if (!existing) {
    return NextResponse.json(
      { error: "Only posts created in admin can be edited here." },
      { status: 404 },
    );
  }
  ...
  const parsed = parsePostBody(body, slug);
  if ("error" in parsed) {
